...
namespace ICSharpCode.AvalonEdit.Editing
{
	...
	static class EditingCommandHandler
	{
		...
		static EditingCommandHandler()
		{
            ...
            CommandBindings.Add(new CommandBinding(AvalonEditCommands.DuplicateLine, OnDuplicateLine));
        ...
        private static void OnDuplicateLine(object target, ExecutedRoutedEventArgs args)
        {
            ...
            {
                DocumentLine currentLine = textArea.Document.GetLineByNumber(textArea.Caret.Line);
                string text = textArea.Document.GetText(!textArea.Selection.IsEmpty ? textArea.Selection.SurroundingSegment : currentLine);
                textArea.Caret.Offset = currentLine.EndOffset;
                textArea.Document.Insert(textArea.Caret.Offset, Environment.NewLine);
                textArea.Document.Insert(textArea.Caret.Offset, text);
                textArea.Caret.Offset = textArea.Document.GetLineByNumber(textArea.Caret.Line).Offset;
                args.Handled = true;
            }
        }

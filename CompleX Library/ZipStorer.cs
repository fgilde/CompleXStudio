...
namespace CompleX_Library
{
    ...
    public class ZipStorer : IDisposable
    {
        ...
        /// <summary>
        /// 	Text encoder for filenames. PKZIP Standard is CP850 (default), ePUB/OCF needs UTF8 (Encoding./UTF8)
        /// </summary>
        public Encoding FilenameEncoder = Encoding.GetEncoding(850);
        ...
        /// </summary>
        public static ZipStorer Create(string filename, string _comment)
        {
        ...
        {
            return new ZipStorer {comment = comment, zipFileStream = stream, access = FileAccess.Write};
        }
        ...
        public static ZipStorer Open(Stream stream, FileAccess access)
        {
            ...
            var zip = new ZipStorer {zipFileStream = stream, access = access};
        ...
        private static string AddDirectorySeparatorChar(string s)
        {
        ...
        {
            AddFolder(Compression.Deflate,path,String.Empty);
        }

        public void AddFolder(string path, string pathNameInZip)
        {
            AddFolder(Compression.Deflate, path, pathNameInZip);
        }

        public void AddFolder(Compression method, string path)
        {
            AddFolder(method, path, String.Empty);            
        }

        public void AddFolder(Compression method, string path, string pathNameInZip)
        {
            foreach (var directory in Directory.GetDirectories(path))
                AddFolder(directory, AddDirectorySeparatorChar(pathNameInZip) + Path.GetFileName(directory)); // or Path.GetFileName(path)

            foreach (var file in Directory.GetFiles(path))
            {
                AddFile(method, file, AddDirectorySeparatorChar(pathNameInZip) + Path.GetFileName(file), String.Empty);
            }
        }

        public void AddFile(Compression method, string pathname)
        {
            AddFile(method,pathname,Path.GetFileName(pathname),String.Empty);
        }
        ...
        /// <param name = "streamcomment">Comment for stored file</param>
        ...
        public void Close()
        {
            if (access != FileAccess.Read)
            {
                ...
                foreach (ZipFileEntry t in files)
                {
        ...
        public List<ZipFileEntry> ReadCentralDir()
        {
            ...
            for (int pointer = 0; pointer < centralDirImage.Length;)
            {
                ...
                var zfe = new ZipFileEntry
                              {
                                  ...
                                  FilenameInZip = FilenameEncoder.GetString(centralDirImage, pointer + 46, filenameSize),
                ...
                if (commentSize > 0)
                    zfe.Comment = FilenameEncoder.GetString(centralDirImage, pointer + 46 + filenameSize + extraSize,
                                                            commentSize);
        ...
        public static bool RemoveEntries(ref ZipStorer zip, List<ZipFileEntry> zfes)
        {
            if (!(zip.zipFileStream is FileStream))
                throw new InvalidOperationException("RemoveEntries just is allowed over streams of type FileStream");
        ...
        */
        ...
        private void WriteLocalHeader(ref ZipFileEntry zfe)
        {
            ...
            zipFileStream.Write(new byte[] {80, 75, 3, 4, 20, 0, 0, 0}, 0, 8); // No extra header
            zipFileStream.Write(BitConverter.GetBytes((ushort) zfe.Method), 0, 2); // zipping method
            zipFileStream.Write(BitConverter.GetBytes(DosTime(zfe.ModifyTime)), 0, 4); // zipping date and time
        ...
        */
        ...
        private void WriteCentralDirRecord(ZipFileEntry zfe)
        {
            ...
            zipFileStream.Write(new byte[] {80, 75, 1, 2, 23, 0xB, 20, 0, 0, 0}, 0, 10);
            zipFileStream.Write(BitConverter.GetBytes((ushort) zfe.Method), 0, 2); // zipping method
            zipFileStream.Write(BitConverter.GetBytes(DosTime(zfe.ModifyTime)), 0, 4); // zipping date and time
        ...
        */
        ...
        {
            byte[] encodedComment = FilenameEncoder.GetBytes(comment);
        ...
        private void Store(ref ZipFileEntry zfe, Stream source)
        {
            ...
            outStream = zfe.Method == Compression.Store ? zipFileStream : new DeflateStream(zipFileStream, CompressionMode.Compress, true);
        ...
        */
        ...
        private static uint DosTime(DateTime dt)
        {
            return
                (uint)
